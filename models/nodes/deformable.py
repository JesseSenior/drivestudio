...
class DeformableNodes(RigidNodes):
    def __init__(self, **kwargs):
        ...
        self.deform_network = ConditionalDeformNetwork(input_ch=3, **self.networks_cfg).to(self.device)
    ...
    def get_deformation(self, local_means) -> Tuple:
        ...
        """
        assert (
            local_means.shape[0] == self.point_ids.shape[0]
    ...
    def deform_gaussian_points(
        ...
        cur_normalized_time = (
            torch.tensor(cur_normalized_time, dtype=torch.float32, device=self.device)
            .unsqueeze(0)
            .repeat(means.shape[0], 1)
        )
