...
class SMPLNodes(RigidNodes):
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor]) -> None:
        ...
        for fi in range(self.num_frames):
            ...
            W, A = self.template(masked_theta=masked_theta, instances_mask=instance_mask)
            T = torch.einsum("bnj, bjrc -> bnrc", W, A)
            R = T[:, :, :3, :3]  # [N, 3, 3]
            t = T[:, :, :3, 3]  # [N, 3]

            reshaped_means = x.reshape(self.num_instances, self.smpl_points_num, 3)
            deformed_means = torch.einsum("bnij,bnj->bni", R, reshaped_means[instance_mask]) + t  # [N, 6890, 3]
    ...
    def transform_means(self, means: torch.Tensor) -> torch.Tensor:
        ...
        if self.in_test_set and (self.cur_frame - 1 > 0 and self.cur_frame + 1 < self.num_frames):
            ...
            masked_theta = torch.where(inter_valid_mask[:, None, None], interpolated_theta, _cur_masked_theta)
        else:
            theta = torch.cat(
                (self.instances_quats[self.cur_frame], self.smpl_qauts[self.cur_frame]),
                dim=1,
        ...
        W, A = self.template(
            ...
            xyz_canonical=means.reshape(self.num_instances, self.smpl_points_num, 3)
            if self.use_voxel_deformer
            else None,
        )
        T = torch.einsum("bnj, bjrc -> bnrc", W, A)
    ...
    def transform_means_and_quats(self, means: torch.Tensor, quats: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        deformed_quats = quat_mult(self.quat_act(R_quats), self.quat_act(reshaped_quats[instance_mask]))
        ...
        quats_container.index_add_(
            ...
            torch.tensor([[[1.0, 0.0, 0.0, 0.0]]], device=self.device).repeat((~instance_mask).sum(), 6890, 1),
        )
    ...
    def load_state_dict(self, state_dict: Dict, **kwargs) -> str:
        ...
        self.instances_quats = Parameter(torch.zeros(self.num_frames, self.num_instances, 1, 4, device=self.device))
        self.smpl_qauts = Parameter(torch.zeros(self.num_frames, self.num_instances, 23, 4, device=self.device))
        self.template = SMPLTemplate(
            smpl_model_path="smpl_models/SMPL_NEUTRAL.pkl",
    ...
    def deform_gaussian_points(
        ...
        cur_frame = torch.argmin(torch.abs(self.normalized_timestamps - cur_normalized_time))
        ...
        W, A = self.template(
            masked_theta=self.quat_act(masked_theta),
            instances_mask=instance_mask,
            xyz_canonical=means.reshape(1, self.smpl_points_num, 3).repeat(self.num_instances, 1, 1)
            if self.use_voxel_deformer
            else None,
        )
        T = torch.einsum("bnj, bjrc -> bnrc", W, A)
