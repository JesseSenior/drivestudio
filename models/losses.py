...
class SafeBCE(autograd.Function):
    ...
    def forward(ctx, x, y, limit):
        assert (torch.where(y != 1, y + 1, y) == 1).all(), "target must all be {0,1}"
        ...
        return -torch.where(
            y == 0,
            torch.log(1 - x).clamp_min_(ln_limit),
            torch.log(x).clamp_min_(ln_limit),
        )
...
def safe_binary_cross_entropy(
    input: torch.Tensor, target: torch.Tensor, limit: float = 0.1, reduction="mean"
) -> torch.Tensor:
...
class DepthLoss(nn.Module):
    ...
    def __call__(
        ...
        if self.depth_error_percentile is not None:
            ...
            depth_error = depth_error[depth_error.argsort()[: int(len(depth_error) * self.depth_error_percentile)]]
