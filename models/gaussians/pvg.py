...
class PeriodicVibrationGaussians(VanillaGaussians):
    ...
    def temporal_means(self):
        ...
        means = self._means + self._velocity * torch.sin((self.cur_time - self._taus) * a) / a
        if self.in_smooth:
    ...
    def temporal_opacities(self):
        return self.get_opacity * torch.exp(-0.5 * (self.cur_time - self._taus) ** 2 / (self.get_scaling_t**2))
    ...
    def refinement_after(self, step, optimizer: torch.optim.Optimizer) -> None:
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                splits_t = (
                    torch.max(self.get_scaling_t, dim=1).values > self.ctrl_cfg.densify_t_size_thresh
                ) & high_t_grads
    ...
    def split_gaussians(self, split_mask, samps):
        ...
        not_split_xyz_mask = (
            self.get_scaling.max(dim=-1).values <= self.ctrl_cfg.densify_size_thresh * self.scene_scale * self.gamma
        ).squeeze()[split_mask]
        new_scales[not_split_xyz_mask.repeat(samps)] = torch.log(self.get_scaling[split_mask].repeat(samps, 1))[
            not_split_xyz_mask.repeat(samps)
        ]

        not_split_t_mask = (torch.max(self.get_scaling_t, dim=1).values <= self.ctrl_cfg.densify_t_size_thresh)[
            split_mask
        ]
        new_betas[not_split_t_mask.repeat(samps)] = torch.log(self.get_scaling_t[split_mask].repeat(samps, 1))[
