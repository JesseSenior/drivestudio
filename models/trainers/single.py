...
class SingleTrainer(BasicTrainer):
            ...
            if class_name == "Background":
                ...
                if num_near_pts + num_far_pts > 0:
                    ...
                    if sampled_time is not None:
                        sampled_time = torch.cat(
                            [
                                sampled_time,
                                torch.zeros(valid_pts.shape[0], 1).to(self.device),
                            ],
                            dim=0,
                        )
                ...
                if isinstance(model, PeriodicVibrationGaussians):
                    model.create_from_pcd(
                        init_means=sampled_pts,
                        init_colors=sampled_color,
                        init_times=sampled_time,
                    )
                else:
                    model.create_from_pcd(init_means=sampled_pts, init_colors=sampled_color)
    ...
    def forward(
        ...
        self.cur_frame = torch.argmin(torch.abs(self.normalized_timestamps - normed_time))
        ...
        gs = self.collect_gaussians(cam=processed_cam, image_ids=image_infos["img_idx"].flatten()[0])
