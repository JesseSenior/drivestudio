...
class ArgoVerseProcessor(object):
    ...
    def save_calib(self, datum: SynchronizedSensorData, scene_idx, frame_idx):
        ...
        for idx, cam_name in enumerate(self.cam_list):
            ...
            np.savetxt(
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/extrinsics/" + f"{str(idx)}.txt",
                c2v,
            )
            np.savetxt(
    ...
    def visualize_dynamic_objects(self, scene_idx, lidar_indices, instances_info, frame_instances):
        ...
        for frame_idx, lidar_idx in tqdm(
            ...
            for cam_name, cam in synchronized_imagery.items():
                ...
                img_path = f"{output_path}/" + f"{str(frame_idx).zfill(3)}_{str(cam_idx)}.jpg"
