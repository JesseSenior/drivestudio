...
from .nuplan_utils import (
    get_egopose3d_for_lidarpc_token_from_db,
    get_tracked_objects_for_lidarpc_token_from_db,
)

NUPLAN_LABELS = ["vehicle", "pedestrian", "bicycle"]
NUPLAN_NONRIGID_DYNAMIC_CLASSES = ["pedestrian", "bicycle"]
NUPLAN_RIGID_DYNAMIC_CLASSES = ["vehicle"]
...
class NuPlanProcessor(object):
    ...
    def __init__(
        ...
        max_frame_limit=300,
        process_keys=["images", "pose", "calib", "lidar", "dynamic_masks", "objects"],
    ...
    def convert_one(self, scene_log_name):
        ...
        if "dynamic_masks" in self.process_keys:
            ...
            self.save_dynamic_mask(
                log_db,
                lidar_idxs,
                valid_classes=NUPLAN_NONRIGID_DYNAMIC_CLASSES,
                dir_name="human",
            )
            self.save_dynamic_mask(
                log_db,
                lidar_idxs,
                valid_classes=NUPLAN_RIGID_DYNAMIC_CLASSES,
                dir_name="vehicle",
            )
    ...
    def save_calib(self, log_db: NuPlanDB, lidar_idxs: List[int]):
        ...
        for channel in self.cam_list:
            ...
            np.savetxt(
                f"{self.save_dir}/{log_db.log_name}/extrinsics/" + f"{str(cam_id)}.txt",
                extrinsics[channel],
            )
            np.savetxt(
                f"{self.save_dir}/{log_db.log_name}/intrinsics/" + f"{str(cam_id)}.txt",
                Ks,
            )
    ...
    def save_pose(self, log_db: NuPlanDB, lidar_idxs: List[int]):
        ...
        for frame_idx, lidar_idx in enumerate(lidar_idxs):
            ...
            np.savetxt(
                f"{self.save_dir}/{log_db.log_name}/ego_pose/" + f"{str(frame_idx).zfill(3)}.txt",
                ego_pose,
            )
            ...
            for channel in self.cam_list:
                ...
                for obj in objects:
                    ...
                    in_image = (
                        ...
                        & np.all(corners_2d[0, :] < self.HW[1])
                        & np.all(corners_2d[1, :] >= 0)
                        & np.all(corners_2d[1, :] < self.HW[0])
                    )
            ...
            for cam_id, image in enumerate(images):
                ...
                if len(objects) == 0:
                    ...
                    for obj_id in objects:
                        ...
                        intersected = np.any(
                            (corners_2d[0, :] >= 0)
                            & (corners_2d[0, :] < self.HW[1])
                            & (corners_2d[1, :] >= 0)
                            & (corners_2d[1, :] < self.HW[0])
                        )
    ...
    def create_folder(self):
        ...
        for scene_log_name in id_list:
            ...
            if "dynamic_masks" in self.process_keys:
                ...
                os.makedirs(
                    f"{self.save_dir}/{scene_log_name}/dynamic_masks/human",
                    exist_ok=True,
                )
                os.makedirs(
                    f"{self.save_dir}/{scene_log_name}/dynamic_masks/vehicle",
                    exist_ok=True,
                )
            if "objects" in self.process_keys:
                ...
                if "objects_vis" in self.process_keys:
                    os.makedirs(
                        f"{self.save_dir}/{scene_log_name}/instances/debug_vis",
                        exist_ok=True,
                    )
