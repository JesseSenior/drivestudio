...
class PandaSetProcessor(object):
        ...
        if "objects" in self.process_keys:
            ...
            for k, v in instances_info.items():
                if v["sibling_id"] != "-":
                    ...
                    if (v["id"], v["sibling_id"]) in duplicated_id_pairs or (
                        v["sibling_id"],
                        v["id"],
                    ) in duplicated_id_pairs:
                        continue
                    else:
            ...
            for pair in duplicated_id_pairs:
                ...
                if pair[0] not in instances_info:
                    ...
                    if len(instances_info[pair[0]]["frame_annotations"]["frame_idx"]) > len(
                        instances_info[pair[1]]["frame_annotations"]["frame_idx"]
                    ):
    ...
    def save_image(self, scene_data: Sequence, scene_idx, frame_idx):
        ...
        for idx, cam in enumerate(self.cam_list):
            img_path = (
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/images/" + f"{str(frame_idx).zfill(3)}_{str(idx)}.jpg"
            )
    ...
    def save_calib(self, scene_data: Sequence, scene_idx, frame_idx):
        ...
        for idx, cam in enumerate(self.cam_list):
            ...
            np.savetxt(
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/extrinsics/" + f"{str(frame_idx).zfill(3)}_{str(idx)}.txt",
                c2w,
            )
            np.savetxt(
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/intrinsics/" + f"{str(idx)}.txt",
                intrinsics,
            )
    ...
    def save_lidar(self, scene_data: Sequence, scene_idx, frame_idx):
        ...
        pcd_ego = geometry.lidar_points_to_ego(pc_world[:, :3], lidar_poses)
        intensity = pc_world[:, 3]
        laser_ids = pc_world[:, 5]

        point_cloud = np.column_stack((pcd_ego, intensity, laser_ids))

        pc_path = f"{self.save_dir}/" + f"{str(scene_idx).zfill(3)}/lidar/{str(frame_idx).zfill(3)}.bin"
    ...
    def save_pose(self, scene_data: Sequence, scene_idx, frame_idx):
        ...
        np.savetxt(
            f"{self.save_dir}/{str(scene_idx).zfill(3)}/ego_pose/" + f"{str(frame_idx).zfill(3)}.txt",
            lidar_to_world,
        )
    ...
    def visualize_3dbox(self, scene_data: Sequence, scene_idx, frame_idx):
        ...
        for idx, cam in enumerate(self.cam_list):
            ...
            # save
            img_path = (
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/3dbox_vis/" + f"{str(frame_idx).zfill(3)}_{str(idx)}.jpg"
            )
    ...
    def visualize_dynamic_objects(
        ...
        for frame_idx in range(num_frames):
            for idx, cam in enumerate(self.cam_list):
                ...
                img_path = f"{output_path}/" + f"{str(frame_idx).zfill(3)}_{str(idx)}.jpg"
                Image.fromarray(img_plotted).save(img_path)
    ...
    def save_objects(self, scene_data: Sequence, num_frames):
        ...
        for frame_idx in range(num_frames):
            ...
            for _, row in cuboids.iterrows():
                ...
                o2w = np.array([[c, -s, 0, tx], [s, c, 0, ty], [0, 0, 1, tz], [0, 0, 0, 1]])
