...
class NuScenesProcessor(object):
    ...
    def convert_one_interpolated(self, scene_idx):
        ...
        if "objects" in self.process_keys:
            ...
            if "objects_vis" in self.process_keys:
                self.visualize_dynamic_objects_interpolated(
                    scene_data,
                    scene_idx,
            ...
            if "dynamic_masks" in self.process_keys:
                self.save_dynamic_mask_interpolated(
                    scene_data,
                    scene_idx,
                    interpolated_timestamps,
                    instances_info,
    ...
    def get_percam_meta(self, scene_data):
        ...
        cams_mata = {cam_name: {"timestamps": [], "is_key_frame": [], "token": []} for cam_name in self.cam_list}
    ...
    def save_calib(self, scene_data, scene_idx):
        ...
        while True:
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                np.savetxt(
                    f"{self.save_dir}/{str(scene_idx).zfill(3)}/intrinsics/{str(cam_idx)}.txt",
                    Ks,
                )
    ...
    def save_lidar(self, scene_data, scene_idx):
        ...
        while True:
            ...
            np.savetxt(
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/lidar_pose/{str(key_frame_idx).zfill(3)}.txt",
                lidar_to_world,
            )
    ...
    def save_lidar_interpolated(self, scene_data, scene_idx, timestamps: np.array):
        ...
        for frame_idx, token in enumerate(closest_tokens):
            ...
            # Save lidar pose in world coordinates
            np.savetxt(
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/lidar_pose/{str(frame_idx).zfill(3)}.txt",
                lidar_to_world,
            )
    ...
    def save_dynamic_mask(self, scene_data, scene_idx, class_valid="all"):
        ...
        while True:
            for cam_idx, cam_name in enumerate(self.cam_list):
                cam_data = self.nusc.get("sample_data", curr_sample_record["data"][cam_name])
                img_path = (
                    f"{self.save_dir}/{str(scene_idx).zfill(3)}/images/{str(key_frame_idx).zfill(3)}_{str(cam_idx)}.jpg"
                )
                ...
                for ann in valid_anns:
                    ...
                    # Fill the mask
                    u, v = (
                        corners_2d[0, :].astype(np.int32),
                        corners_2d[1, :].astype(np.int32),
                    )
    ...
    def save_dynamic_mask_interpolated(self, scene_data, scene_idx, timestamps, instances_info, class_valid="all"):
        ...
        for frame_idx, timestamp in enumerate(timestamps):
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                img_path = (
                    f"{self.save_dir}/{str(scene_idx).zfill(3)}/images/{str(frame_idx).zfill(3)}_{str(cam_idx)}.jpg"
                )
    ...
    def save_objects(self, scene_data):
        ...
        while True:
            ...
            frame_instances[key_frame_idx].extend(
                [ann["instance_token"] for ann in anns if ann["category_name"] in NUSCENES_DYNAMIC_CLASSES]
            )
            ...
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                if len(objects) > 0:
                    ...
                    for obj_id in objects:
                        ...
                        o2w = np.array(instances_info[obj_id]["frame_annotations"]["obj_to_world"][idx_in_obj])
                ...
                img_path = f"{output_path}/" + f"{str(key_frame_idx).zfill(3)}_{str(cam_idx)}.jpg"
    ...
    def visualize_dynamic_objects_interpolated(
        self, scene_data, scene_idx, timestamps, instances_info, frame_instances
    ):
    ...
    def create_folder(self):
        ...
        for i in id_list:
            ...
            if "dynamic_masks" in self.process_keys:
                os.makedirs(
                    f"{self.save_dir}/{str(i).zfill(3)}/dynamic_masks/all",
                    exist_ok=True,
                )
                os.makedirs(
                    f"{self.save_dir}/{str(i).zfill(3)}/dynamic_masks/human",
                    exist_ok=True,
                )
                os.makedirs(
                    f"{self.save_dir}/{str(i).zfill(3)}/dynamic_masks/vehicle",
                    exist_ok=True,
                )
            if "objects" in self.process_keys:
