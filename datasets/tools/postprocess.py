...
def match_and_postprocess(
    ...
    for cam_id in camera_list:
        ...
        for fi, pred_track_f in enumerate(raw_pred_track.values()):
            for i_, tid in enumerate(pred_track_f["tid"]):
                ...
                parsed_pred_track[tid]["smpl"]["global_orient"][fi] = torch.tensor(
                    pred_track_f["smpl"][i_]["global_orient"]
                )
            ...
            for gt_tid in parsed_gt_track.keys():
                ...
                for fi in range(num_f):
                    if parsed_pred_track[pred_tid]["valids"][fi] and parsed_gt_track[gt_tid]["valids"][fi]:
                        iou += compute_iou(
                            parsed_pred_track[pred_tid]["tracked_bbox"][fi],
                            parsed_gt_track[gt_tid]["tracked_bbox"][fi],
                        )
                iou /= num_f
                if iou > max_iou:
                    max_iou = iou
            ...
            for gt_tid in valid_gt_tids:
                ...
                for fi in range(num_f):
                    if parsed_gt_track[gt_tid]["valids"][fi] and parsed_pred_track[matched_pred_tid]["valids"][fi]:
                        collector[gt_tid]["smpl"]["global_orient"][cam_id, fi] = parsed_pred_track[matched_pred_tid][
                            "smpl"
                        ]["global_orient"][fi].clone()
                        collector[gt_tid]["smpl"]["body_pose"][cam_id, fi] = parsed_pred_track[matched_pred_tid][
                            "smpl"
                        ]["body_pose"][fi].clone()
                        collector[gt_tid]["smpl"]["betas"][cam_id, fi] = parsed_pred_track[matched_pred_tid]["smpl"][
                            "betas"
                        ][fi].clone()
                        collector[gt_tid]["camera"][cam_id, fi] = parsed_pred_track[matched_pred_tid]["camera"][
                            fi
                        ].clone()
    ...
    if save_temp:
        for cam_id in camera_list:
            ...
            for fi in range(num_f):
                ...
                pkl_dict[fi] = fi_info
            joblib.dump(pkl_dict, os.path.join(temp_dir, f"{cam_id}_matched.pkl"))
    ...
    if save_temp:
        for cam_id in camera_list:
            ...
            for fi in range(num_f):
                ...
                for gt_tid in valid_gt_tids:
                    if collector[gt_tid]["2DBox_appear_mask"][
                        cam_id, fi
                    ]:  # the difference is we use 2DBox_appear_mask here
                        fi_info["tid"].append(gt_tid)
                        fi_info["tracked_time"].append(0)
                        fi_info["smpl"].append(
                            {
                        ...
                        fi_info["camera"].append(collector[gt_tid]["camera"][cam_id, fi].numpy())
                pkl_dict[fi] = fi_info
            joblib.dump(pkl_dict, os.path.join(temp_dir, f"{cam_id}_completed.pkl"))
    ...
    for gt_tid in valid_gt_tids:
        ...
        merged_collector[gt_tid]["smpl"]["global_orient"][mask] = collector[gt_tid]["smpl"]["global_orient"][:, mask][
            masked_best_cam_id, torch.arange(masked_best_cam_id.shape[0])
        ]
        merged_collector[gt_tid]["smpl"]["body_pose"][mask] = collector[gt_tid]["smpl"]["body_pose"][:, mask][
            masked_best_cam_id, torch.arange(masked_best_cam_id.shape[0])
        ]
        merged_collector[gt_tid]["smpl"]["betas"][mask] = collector[gt_tid]["smpl"]["betas"][:, mask][
            masked_best_cam_id, torch.arange(masked_best_cam_id.shape[0])
        ]
    ...
    for gt_tid in valid_gt_tids:
        need_fill = torch.tensor(detect_breaks_mask(merged_collector[gt_tid]["must_appear_mask"]))
        if need_fill.any():
    ...
    if save_temp:
        ...
        for cam_id in camera_list:
            for fi in range(num_f):
                ...
                pkl_dict[fi] = fi_info
            joblib.dump(pkl_dict, os.path.join(temp_dir, f"{cam_id}_merged.pkl"))
