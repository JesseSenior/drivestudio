...
def run_4DHumans(
    ...
    if not verbose:
        ...
        files_to_remove = [os.path.join(temp_dir, "phalp_output", f"PHALP_{i}.mp4") for i in camera_list]
    ...
    for i, cam_id in enumerate(camera_list):
        if already_done:
            pred_tracks_allcam[cam_id] = joblib.load(os.path.join(temp_dir, "phalp_output", f"cam_{cam_id}.pkl"))
        else:
            pred_tracks_allcam[cam_id] = joblib.load(os.path.join(temp_dir, "phalp_output", "results", f"demo_{i}.pkl"))
