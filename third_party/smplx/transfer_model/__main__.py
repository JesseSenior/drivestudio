...
from .utils import np_mesh_to_o3d, read_deformation_transfer
...
def main() -> None:
    ...
    if torch.cuda.is_available() and exp_cfg["use_cuda"]:
        ...
        if exp_cfg["use_cuda"]:
            if (
                input("use_cuda=True and GPU is not available, using CPU instead, would you like to continue? (y/n)")
                != "y"
            ):
                sys.exit(3)
