...
from .utils import Tensor, apply_deformation_transfer, batch_rodrigues, get_vertices_per_edge
...
def summary_closure(gt_vertices, var_dict, body_model, mask_ids=None):
    ...
    for key, var in var_dict.items():
        ...
        if "pose" in key or "orient" in key:
            param_dict[key] = batch_rodrigues(var.reshape(-1, 3)).reshape(len(var), -1, 3, 3)
        else:
...
def build_model_forward_closure(
    ...
    if per_part:
        ...
        assert cond, 'When per-part is True, "part", "part_key", "jidx" must not be None.'
        ...
        def model_forward():
            ...
            for key, var in var_dict.items():
                if part_key == key:
                    param_dict[key] = batch_rodrigues(var.reshape(-1, 3)).reshape(len(var), -1, 3, 3)
                    param_dict[key][:, jidx] = batch_rodrigues(part.reshape(-1, 3)).reshape(-1, 3, 3)
                else:
                    # Decode the axis-angles
                    if "pose" in key or "orient" in key:
                        param_dict[key] = batch_rodrigues(var.reshape(-1, 3)).reshape(len(var), -1, 3, 3)
...
def get_variables(batch_size: int, body_model: nn.Module, dtype: torch.dtype = torch.float32) -> Dict[str, Tensor]:
    var_dict = {}
    ...
    if per_part:
        for key, var in tqdm(var_dict.items(), desc="Parts"):
            ...
            for jidx in tqdm(range(var.shape[1]), desc="Joints"):
                part = torch.zeros([batch_size, 3], dtype=dtype, device=device, requires_grad=True)
